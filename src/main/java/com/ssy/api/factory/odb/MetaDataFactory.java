...
public class MetaDataFactory {
	...
	protected static Map<String, File> loadIntfWordFileMap() {
		if (CommUtil.isNull(intfExcelFileMap) && CommUtil.isNotNull(sdtContextConfig.getIntfExcelPath())) {
			...
			intfExcelFileMap = loaderFactory.getFileLoader().load(sdtContextConfig.getIntfExcelPath(), false,
					SdtConst.INTF_EXCEL_SUFFIX);
	...
	protected static Map<String, File> loadProjectFileMap() {
		if (CommUtil.isNull(projectFileMap)) {
			StopWatch s = BizUtil.startStopWatch();
			projectFileMap = loaderFactory.getFileLoader().load(sdtContextConfig.getWorkSpacePath(), true,
					SdtConst.PROJECT_FILE_SUFFIX);
					...
					if (c.isDict()) {
						c.getElementMap().forEach((id, now) -> {
							...
							if (null == before) {
								...
								dictMap.put(id, loaderFactory.getComplexTypeLoader().checkDictPriorty(dictPriortyMap,
										before, now));
							}
						});
					}
				...
				eMap.forEach((key, e) -> {
					if (e.getRestriction() == E_RESTRICTION.ENUMTYPE) {
						...
						if (null == before) {
							...
							enumMap.put(key,
									loaderFactory.getRestrictionLoader().checkEnumPriorty(enumPriortyMap, before, e));
						}
					}
				});
			});
		...
		loadProjectFileMap().forEach((fileName, file) -> {
			...
			try {
				...
				/** flowtran、数据库表、报表、服务 **/
						...
						|| fileName.contains(SdtConst.REPORT_SUFFIX)
						|| fileName.contains(SdtConst.SERVICETYPE_SUFFIX)) {
	 ...
	 * @param replaceMap
	 */
	private static void metaDataVerify(File file, Map<String, String> replaceMap, String elementName,
			Map<String, SdpDictPriorty> dictPriortyMap, Map<String, TwoTuple<String, String>> customReplaceMap)
		...
		XmlParser.searchTargetAllXmlElement(document.getRootElement(), elementName).forEach(e -> {
			...
			if (CommUtil.isNotNull(dict)) {
				...
				if (CommUtil.isNull(replaceTwoTuple)
						&& !CommUtil.equals(file.getName().toLowerCase().substring(0, 2), groupId)
						&& !CommUtil.equals(groupId, SdtConst.WILDCARD)) {
					return;
				}
				...
				if (CommUtil.isNotNull(refAttribute) && CommUtil.isNotNull(refAttribute.getValue())) {
					...
					if (!CommUtil.equals(beforeValue, refAttribute.getValue())) {
						log.info("[{}]The reference of element [{}] is updated {}->{}", file.getName(), dict.getId(),
								beforeValue, refAttribute.getValue());
					}
				}
				// 类型
				Attribute typeAttribute = file.getName().contains(SdtConst.NAMEDSQL_SUFFIX) ? e.attribute("javaType")
						: e.attribute("type");
				if (CommUtil.isNotNull(dict.getType()) && CommUtil.isNotNull(typeAttribute)
						&& CommUtil.isNotNull(typeAttribute.getValue())) {
					...
					if (!CommUtil.equals(typeAttribute.getValue(), dict.getType().getFullId())
							&& CommUtil.isNull(replaceTwoTuple)) {
					...
					if (!CommUtil.equals(beforeValue, typeAttribute.getValue())) {
						log.info("[{}]The type of element [{}] is updated {}->{}", file.getName(), dict.getId(),
								beforeValue, typeAttribute.getValue());
					}
				}
	...
	private static void replaceStrByMap(File file, Map<String, String> map) throws IOException {
		String content = loaderFactory.getFileLoader().loadAsString(file, SdtConst.DEFAULT_CHARSET.name());
