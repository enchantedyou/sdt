...
public class DefaultDataSourceLoader implements DataSourceLoader {
	...
	private void testDruidDataSourceValid(String dataSourceId, DruidDataSource druidDataSource) {
		...
		try {
			...
			connection = DriverManager.getConnection(druidDataSource.getUrl(), druidDataSource.getUsername(),
					druidDataSource.getPassword());
		} catch (Exception e) {
			throw new SdtException("Failed to create the data source connection for [" + dataSourceId + "],cause by: "
					+ e.getMessage());
		} finally {
			if (null != connection) {
				try {
					...
					log.info("Failed to close the data source connection");
				}
			}
		}
	}
