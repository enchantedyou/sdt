...
public interface ApsAccountingEventMapper {
	int deleteByPrimaryKey(@Param("trxnSeq") String trxnSeq, @Param("dataSort") Long dataSort,
	...
	ApsAccountingEvent selectByPrimaryKey(@Param("trxnSeq") String trxnSeq, @Param("dataSort") Long dataSort,
			@Param("orgId") String orgId);
	...
	BigDecimal selSumAmtByBalAttribute(@Param("balAttributes") String balAttributes, @Param("acctNo") String acctNo,
			@Param("debitCredit") String debitCredit);

	/** 根据流水和账号获取会计事件分录 **/
	List<ApsAccountingEvent> selectAll(@Param("acctNo") String acctNo, @Param("orgId") String orgId);
}