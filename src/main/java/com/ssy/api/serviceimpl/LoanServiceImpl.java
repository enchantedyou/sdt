...
public class LoanServiceImpl implements LoanService {
	...
	public List<String> checkLoanBalance() {
		...
		if (CommUtil.isNull(currentDataSource) || !currentDataSource.getUrl().contains("ln")) {
			throw new SdtException("The current data source[{}] is illegal, it is not a valid loan data source",
					dataSource);
		}
		...
		List<LnpBalAttributes> balAttributesList = balAttributesMapper.selectAll(contextConfig.getBusiOrgId(),
				true);
		...
		// 借据账务余额检查
		List<LnaLoan> loanList = lnaLoanMapper.selectAll(contextConfig.getBusiOrgId());
		for (LnaLoan lnaLoan : loanList) {
			List<ApsAccountingEvent> accountingEventList = accountingEventMapper.selectAll(lnaLoan.getLoanNo(),
					contextConfig.getBusiOrgId());
			List<LnaBalance> balanceList = lnaBalanceMapper.selectAll(lnaLoan.getLoanNo(),
					contextConfig.getBusiOrgId());
			...
			for (ApsAccountingEvent accountingEvent : accountingEventList) {
				...
				if (CommUtil.isNull(bal)) {
					...
					// 同向相加反向相减
					accountingBalMap.put(balAttr,
							(CommUtil.equals(debitCredit, balAttributesGlMap.get(balAttr).getDebitCredit()))
									? bal.add(trxnAmt)
									: bal.subtract(trxnAmt));
				}
			}

			// 汇总账户余额
			if (CommUtil.isNull(balanceList)) {
				log.warn("Loan number[{}] is dirty data for data source[{}]", lnaLoan.getLoanNo(),
			...
			if (accountingBalMap.size() != balanceBalMap.size()) {
				...
				log.warn(
						"The number of balance attributes[{}] in the accounting event is inconsistent with the number of attributes of the loan balance[{}]",
						accountingBalMap.size(), balanceBalMap.size());
			} else {
				for (Map.Entry<String, BigDecimal> entry : accountingBalMap.entrySet()) {
					...
					if (!CommUtil.equals(accountingBal, balanceBal)) {
						errorLoanList.add(lnaLoan.getLoanNo());
						log.warn(
								"The amount[{}] corresponding to the accounting event corresponding to the balance attribute[{}] of the loan[{}] is inconsistent with the amount[{}] in the balance table",
								accountingBal, entry.getKey(), lnaLoan.getLoanNo(), balanceBal);
						break;
					}
				}
			}
	...
	public List<LnaLoan> queryLoanList() {
		return lnaLoanMapper.selectAll(contextConfig.getBusiOrgId());
	}
}
